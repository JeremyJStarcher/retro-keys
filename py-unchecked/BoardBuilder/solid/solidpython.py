...
def scad_render_animated(
    func_to_animate, steps=20, back_and_forth=True, filepath=None, file_header=""
):
...
def scad_render_to_file(
    scad_object, filepath=None, file_header="", include_orig_code=True
):
...
def parse_scad_callables(scad_code_str):
    ...
    args_re = (
        r"(?mxs)(?P<arg_name>\w+)(?:\s*=\s*(?P<default_val>[\w.-]+|\[.*\]))?(?:,|$)"
    )
...
def new_openscad_class_str(
    class_name, args=[], kwargs=[], include_file_path=None, use_not_include=True
):
    args_str = ""
    args_pairs = ""
    ...
    if include_file_path:
        ...
        result = (
            "class %(class_name)s(OpenSCADObject):\n"
...
class IncludedOpenSCADObject(OpenSCADObject):
    ...
    def _get_include_path(self, include_file_path):
        ...
        raise ValueError(
            "Unable to find included SCAD file: "
