...
other_test_cases = [
    ...
    (
        "euclidify_recursive_Vector",
        euclidify,
        [[Vector3(0, 0, 0), Vector3(0, 0, 1)]],
        "[Vector3(0.00, 0.00, 0.00), Vector3(0.00, 0.00, 1.00)]",
    ),
    (euc_to_arr, [Vector3(0, 0, 0)], "[0, 0, 0]"),
    (
        "euc_to_arr_recursive",
        euc_to_arr,
        [[Vector3(0, 0, 0), Vector3(0, 0, 1)]],
        "[[0, 0, 0], [0, 0, 1]]",
    ),
    ...
    (
        "transform_to_point_single_arr",
        transform_to_point,
        [[1, 0, 0], [2, 2, 2], [3, 3, 1]],
        "Point3(2.71, 1.29, 2.00)",
    ),
    (
        "transform_to_point_single_pt3",
        transform_to_point,
        [Point3(1, 0, 0), [2, 2, 2], [3, 3, 1]],
        "Point3(2.71, 1.29, 2.00)",
    ),
    (
        "transform_to_point_arr_arr",
        transform_to_point,
        [[[1, 0, 0], [0, 1, 0], [0, 0, 1]], [2, 2, 2], [3, 3, 1]],
        "[Point3(2.71, 1.29, 2.00), Point3(1.84, 1.84, 2.97), Point3(1.31, 1.31, 1.77)]",
    ),
    (
        "transform_to_point_pt3_arr",
        transform_to_point,
        [[Point3(1, 0, 0), Point3(0, 1, 0), Point3(0, 0, 1)], [2, 2, 2], [3, 3, 1]],
        "[Point3(2.71, 1.29, 2.00), Point3(1.84, 1.84, 2.97), Point3(1.31, 1.31, 1.77)]",
    ),
    (
        "transform_to_point_redundant",
        transform_to_point,
        [
        ...
        "[Point3(2.00, 2.00, 2.00), Point3(-8.00, 2.00, 2.00), Point3(2.00, 12.00, 2.00)]",
    ),
    (
        "offset_points_inside",
        offset_points,
        [tri, 2, True],
        "[Point3(2.00, 2.00, 0.00), Point3(5.17, 2.00, 0.00), Point3(2.00, 5.17, 0.00)]",
    ),
    (
        "offset_points_outside",
        offset_points,
        [tri, 2, False],
        "[Point3(-2.00, -2.00, 0.00), Point3(14.83, -2.00, 0.00), Point3(-2.00, 14.83, 0.00)]",
    ),
    (
        "offset_points_open_poly",
        offset_points,
        [tri, 2, False, False],
        "[Point3(0.00, -2.00, 0.00), Point3(14.83, -2.00, 0.00), Point3(1.41, 11.41, 0.00)]",
    ),
]
...
class TestSPUtils(DiffOutput):
    ...
    def test_split_body_planar(self):
        ...
        for split_dir in [RIGHT_VEC, FORWARD_VEC, UP_VEC]:
            actual_tuple = split_body_planar(
                body, body_bb, cutting_plane_normal=split_dir, cut_proportion=0.25
            )
