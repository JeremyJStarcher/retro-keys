...
def arc_inverted(rad, start_degrees, end_degrees, segments=None):
    ...
    if end_degrees - start_degrees == 180:
        raise ValueError(
            "Unable to draw inverted arc over 180 or more "
            "degrees. start_degrees: %s end_degrees: %s" % (start_degrees, end_degrees)
        )
    ...
    bottom_half_square = translate([-(wide - rad), -high])(
        square([wide, high], center=False)
    )
...
class BoundingBox(object):
    ...
    def split_planar(
        self, cutting_plane_normal=RIGHT_VEC, cut_proportion=0.5, add_wall_thickness=0
    ):
        cpd = {
            RIGHT_VEC: 0,
            LEFT_VEC: 0,
    ...
    def cube(self, larger=False):
        ...
        c = translate(self.position())(cube(c_size, center=True))
        return c
...
# = Model Splitting =
# ===================
def split_body_planar(
    obj,
    obj_bb,
    cutting_plane_normal=UP_VEC,
    cut_proportion=0.5,
    dowel_holes=False,
    dowel_rad=4.5,
    hole_depth=15,
    add_wall_thickness=0,
):
    ...
    if dowel_holes:
        ...
        cut_point = (
            part_bbs[0].position()[cutting_plane]
            + part_bbs[0].size()[cutting_plane] / 2
        )
...
def bom_part(description="", per_unit_price=None, currency="US$", *args, **kwargs):
    def wrap(f):
        ...
        elements = {}
        elements.update(
            {"Count": 0, "currency": currency, "Unit Price": per_unit_price}
        )
...
def screw(screw_type="m3", screw_length=16):
    ...
    ret = union()(
        ...
        up(screw_length)(cylinder(cap_rad, cap_height)),
    )
    return ret
...
try:
        ...
        if isinstance(an_obj, (list, tuple)):
            if isinstance(an_obj[0], (list, tuple)):
                ...
                try:
                    ...
                    raise TypeError(
                        "Object: %s ought to be PyEuclid class %s or "
    ...
    def euc_to_arr(euc_obj_or_list):  # Inverse of euclidify()
        ...
        elif isinstance(euc_obj_or_list, (list, tuple)) and hasattr(
            euc_obj_or_list[0], "as_arr"
        ):
    ...
    # = Transforms =
    # ==============
    def transform_to_point(
        body,
        dest_point,
        dest_normal,
        src_point=Point3(0, 0, 0),
        src_normal=Vector3(0, 1, 0),
    ...
    def offset_points(point_arr, offset, inside=True, closed_poly=True):
        ...
        if closed_poly:
            for i in range(len(point_arr) - 1):
                a, b = point_arr[i : i + 2]
                par_seg = _parallel_seg(
                    a, b, normal=normal, offset=offset, direction=direction
                )
                segs.append(par_seg)
                if len(segs) > 1:
    ...
    def _inside_direction(a, b, c, offset=10):
        ...
        l_segs = [
            _parallel_seg(p, q, normal=x, offset=offset, direction=LEFT)
            for p, q in ((a, b), (b, c))
        ]
        r_segs = [
            _parallel_seg(p, q, normal=x, offset=offset, direction=RIGHT)
            for p, q in ((a, b), (b, c))
        ]
    ...
    def fillet_2d(three_point_sets, orig_poly, fillet_rad, remove_material=True):
        ...
        if len(three_point_sets) == 3 and isinstance(
            three_point_sets[0], (Vector2, Vector3)
        ):
        ...
        for which_loop in range(len(path_pts)):
            ...
            if which_loop < len(path_pts) - 1:
                ...
                facet_indices.append(
                    [
                        segment_start,
                        segment_end + shape_pt_count,
                        segment_start + shape_pt_count,
                    ]
                )
