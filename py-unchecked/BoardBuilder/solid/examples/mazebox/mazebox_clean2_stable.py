...
def bumpMapCylinder(theR, hn, inset, default):
    ...
    for j in range(0, rn):
        ...
        for i in range(0, hn - 1):
            ...
            p3 = (i + 1) * rn + j
            p4 = (i + 1) * rn + ((j + 1) % rn)
            a1 = angleBetweenPlanes(
                [pts[p1], pts[p2], pts[p3]], [pts[p4], pts[p1], pts[p3]]
            )
            a1 = min(a1, pi - a1)
            a2 = angleBetweenPlanes(
                [pts[p2], pts[p1], pts[p4]], [pts[p2], pts[p3], pts[p4]]
            )
            a2 = min(a2, pi - a2)
            # print(a1, a2)
            if a1 < a2:
...
if part == 1:
    ...
    d.add(
        intersection()
        .add(bumpMapCylinder(innerR, hn + 2, wall, 0).set_modifier(""))
        .add(
            translate([0, 0, baseH]).add(
                cylinder(r=innerR + 2 * wall, h=h * 1.1).set_modifier("")
            )
        )
    )
    # u.add()
    ...
    for i in range(0, 3):
        ...
        r = innerR + gap + wall / 2
        u.add(
            translate([(r - 0.3) * cos(a), (r - 0.3) * sin(a), toph - 6]).add(
                sphere(r=2.4)
            )
        )
        u2.add(
            translate(
                [(r + wall - 0.3) * cos(a), (r + wall - 0.3) * sin(a), toph - 6]
            ).add(sphere(r=2.4))
        )
    ...
    for i in range(0, 3):
        ...
        u2.add(
            translate([(r + wall) * cos(a), (r + wall) * sin(a), 4]).add(sphere(r=2.3))
        )
    # top.add(cylinder(r = innerR+wall+gap, h=h))
    print("//$fn=20;\n")
    print(top._render())
