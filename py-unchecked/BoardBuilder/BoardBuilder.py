...
class BoardBuilder:
    def __init__(
        ...
        try:
            ...
            if max_wall == "min_pad":
                ...
                print(
                    "WARNING: Unknown value for max_wall parameter: {0}. Defaulting to {1}".format(
                        max_wall, max_wall_thickness
                    )
                )
        ...
        self.mid_layer_closed_sectioned = self.build_sectioned_mid_layer(
            self.mid_layer_closed
        )
    ...
    def apply_corners(self, plate):
        ...
        return difference()(
            plate,
            build_corner(),
            translate([self.exterior_width, 0, 0])(mirror([1, 0, 0])(build_corner())),
            translate([self.exterior_width, self.exterior_height, 0])(
                mirror([1, 1, 0])(build_corner())
            ),
            translate([0, self.exterior_height, 0])(mirror([0, 1, 0])(build_corner())),
        )
    ...
    def apply_screw_holes(self, plate):
        def build_screw_hole_row(y, row_wall_thickness, row_is_top):
            ...
            # Get the hole radius
            hole_radius = self.hole_diameter / 2.0
            ...
            start_y_adjustment = (
                start_x if self.left_wall_thickness > row_wall_thickness else 0
            )
            end_y_adjustment = (
                end_x_adjustment
                if self.right_wall_thickness > row_wall_thickness
                else 0
            )
    ...
    def switch_hole(self, width_factor, height_factor, stab_style):
        def stab_geometry():
            ...
            def cherry_stab():
                ...
                stab_hole = union()(
                    translate([0, -6.77, 0])(
                        translate([-3.325, 0, 0])(square(size=[6.65, 12.3])),
                        # Bottom notch.
                        translate([-1.5, -1.2, 0])(square(size=[3.0, 2])),
                    ),
                    ...
                    translate([0, -0.5, 0])(square(size=[4.2, 2.8])),
                )
            ...
            bottom_fudge = (
                stab_style["bottom_fudge"]
                if "bottom_fudge" in stab_style
                else self.stab_vertical_adjustment
            )
            height_fudge = (
                stab_style["height_fudge"]
                if "height_fudge" in stab_style
                else self.stab_height_adjustment
            )
            ...
            if stab_style["type"] == "cherry":
                ...
                return costar_stab(bottom_fudge=bottom_fudge, height_fudge=height_fudge)
                ...
                bottom_fudge += (
                    -0.22
                )  # This lowers the costar cutout to align with the bottom cherry notch.
                # Use height_fudge to adjust the costar clips' top offsets.
                # Or a Dremel.

                return union()(
                    cherry_stab(),
                    costar_stab(bottom_fudge=bottom_fudge, height_fudge=height_fudge),
                )
        ...
        def build_stab(a, left=None, right=None):
            ...
            if stab_style["type"] in ("both", "cherry"):
                ...
                if a < 24:
                    stab = union()(
                        stab, translate([-11.9, -5.97, 0])(square(size=[23.8, 10.7]))
                    )
        ...
        if width_factor >= 2.0 or height_factor >= 2.0:
            ...
            if width_factor >= 8.0 or height_factor >= 8.0:
                ...
                stab = build_stab(
                    133.35
                )  # == 5.25 inches.  Metric and imperial specs match.
            ...
            # TODO: Deal with this asymmetrical case.
                ...
                stab = build_stab(
                    38.1
                )  # == 1.5 inches.  Metric and imperial specs match here...
            ...
            hole = union()(hole, mirror([0, 1, 0])(stab))
    ...
    def build_base_top_plate(self):
            ...
            if type(row) == list:
                for e in row:
                    ...
                    if type(e) == dict:
                        ...
                        if (
                            "x" in e
                        ):  # Forcing the cursor's x positioning relative to "here."
                            cursor_x = cursor_x + (standard_key_spacing * e["x"])
                        if (
                            "y" in e
                        ):  # Forcing the cursor's y positioning relative to "here."
                            cursor_y = cursor_y + (standard_key_spacing * e["y"])
                        if "d" in e:  # Next "key" is really a decal.  Skip it.
                            skip_next = True
                        if (
                            "bb_stab" in e
                        ):  # BoardBuilder-specific wrinkle: force stabilizer style.
                            next_stab = e["bb_stab"]
                        ...
                        def rotate2d(degrees, point):
                            ...
                            return [
                                point[0] * math.cos(rads) - point[1] * math.sin(rads),
                        ...
                        def apply_square():
                            ...
                            return [
                                apply_transform(
                                    [-space_relative_x_center, -space_relative_y_center]
                                ),
                                apply_transform(
                                    [space_relative_x_center, -space_relative_y_center]
                                ),
                                ...
                                return trans2d(
                                    ...
                                    rotate2d(
                                        ...
                                        trans2d(
                                            ...
                                            trans2d(
                                                [
                                                    cursor_x + space_relative_x_center,
                                                    cursor_y + space_relative_y_center,
                                                ],
                                                point,
                                            ),
                                        ),
                                    ...
                                    rotate([0, 0, r])(
                                        translate([-rx, -ry, 0])(
                                            translate(
                                                ...
                                                self.switch_hole(
                                                    next_key_width_factor,
                                                    next_key_height_factor,
                                                    next_stab,
                                                )
                                            )
                                        )
        ...
        def transform_from_kle_geometry(geometry):
            return translate([0, self.interior_height, 0])(
                mirror([0, 1, 0])(translate([-self.min_x, -self.min_y, 0])(geometry))
            )
        ...
        self.holes = transform_from_kle_geometry(key_hole_squares)
        ...
        plate = difference()(
            ...
            translate([self.left_pad, self.bottom_pad, 0])(self.holes),
        )

        if self.show_points:
            point_collection = [
                translate([p[0], p[1], 1])(circle(r=1, segments=20))
                for p in key_space_points
            ]
            plate = union()(
                plate,
                color("red")(
                    translate([self.left_pad, self.bottom_pad, 0])(
                        transform_from_kle_geometry(point_collection)
                    )
                ),
            )
    ...
    def build_mid_layers(self, plate):
        ...
        padding_rectangle = square(
            size=[
                self.exterior_width
                - (self.left_wall_thickness + self.right_wall_thickness),
                self.exterior_height
                - (self.top_wall_thickness + self.bottom_wall_thickness),
            ]
        )

        return difference()(
            plate,
            translate([self.left_pad, self.bottom_pad, 0])(interior_rectangle),
    ...
    def build_sectioned_mid_layer(self, mid_layer):
        if mid_layer:
            ...
            def mid_layer_quadrant(horizontal_quadrant, vertical_quadrant):
                ...
                section = translate(
                    [
                        -horizontal_offset - half_width / 2,
                        -vertical_offset - half_height / 2,
                        0,
                    ]
                )(
                    intersection()(
            ...
            space_optimized_mid_layer = union()(
                ...
                translate(
                    [self.left_wall_thickness + 3, self.bottom_wall_thickness + 3, 0]
                )(
                    mid_section_lower_right,
                    translate(
                        [
                            self.right_wall_thickness + 3,
                            self.bottom_wall_thickness + 3,
                            0,
                        ]
                    )(
                        ...
                        translate(
                            [
                                self.left_wall_thickness + 3,
                                self.top_wall_thickness + 3,
                                0,
                            ]
    ...
    def render_top_plate(self, output_dir):
        scad_render_to_file(
            self.base_top_plate,
            os.path.join(output_dir, "top.scad"),
            include_orig_code=False,
        )
        scad_render_to_file(
            self.holes, os.path.join(output_dir, "holes.scad"), include_orig_code=False
        )

    def render_bottom_plate(self, output_dir):
        scad_render_to_file(
            self.base_bottom_plate,
            os.path.join(output_dir, "bottom.scad"),
            include_orig_code=False,
        )
    ...
    def render_mid_layers(self, output_dir):
        if self.mid_layer_closed:
            scad_render_to_file(
                self.mid_layer_closed,
                os.path.join(output_dir, "mid_closed.scad"),
                include_orig_code=False,
            )

        if self.mid_layer_closed_sectioned:
            scad_render_to_file(
                self.mid_layer_closed_sectioned,
                os.path.join(output_dir, "mid_closed_sectioned.scad"),
                include_orig_code=False,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="0.0",
        help="Horizontal padding per side. Can also define left,right padding.",
    )
    parser.add_argument(
        "-vp",
        "--vertical_pad",
        type=str,
        default="0.0",
        help="Vertical padding per side. Can also define top,bottom padding.",
    )
    parser.add_argument(
        "-mw",
        "--max_wall",
        type=str,
        default="10.0",
        help="Max mid-layer wall thickness. Can also be 'min_pad' or 'max_pad'",
    )
    parser.add_argument(
        "-c", "--corner_radius", type=float, default=0.0, help="Corner radius."
    )
    parser.add_argument(
        "-n", "--num_holes", type=int, default=0, help="Number of screw holes."
    )
    parser.add_argument(
        "-hd", "--hole_diameter", type=float, default=0.0, help="Screw hole diameter."
    )
    parser.add_argument(
        "-sp",
        "--show_points",
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="How many sides to put on the screw holes. 20 is good for circles, 6 would be hex.",
    )

    parser.add_argument(
        "-sva",
        "--stab_vertical_adjustment",
        type=float,
        default=0.0,
        help="Adjust the vertical positioning of Costar stabs.",
    )
    parser.add_argument(
        "-sha",
        "--stab_height_adjustment",
        type=float,
        default=0.0,
        help="Adjust the vertical size of Costar stabs.",
    )
