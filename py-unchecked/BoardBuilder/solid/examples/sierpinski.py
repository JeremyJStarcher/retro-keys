...
class SierpinskiTetrahedron(object):
    ...
    def next_gen(self, midpoint_weight=0.5, jitter_range_vec=None):
        midpoints = [
            weighted_midpoint(
                s[0], s[1], weight=midpoint_weight, jitter_range_vec=jitter_range_vec
            )
...
def weighted_midpoint(a, b, weight=0.5, jitter_range_vec=None):
    ...
    if jitter_range_vec:
        x += (random.random() - 0.5) * dist * jitter_range_vec[0]
        y += (random.random() - 0.5) * dist * jitter_range_vec[1]
        z += (random.random() - 0.5) * dist * jitter_range_vec[2]

    return [x, y, z]


def sierpinski_3d(generation, scale=1, midpoint_weight=0.5, jitter_range_vec=None):
    orig_tet = SierpinskiTetrahedron(
    ...
    for i in range(generation):
        all_tets = [
            subtet
            for tet in all_tets
            for subtet in tet.next_gen(midpoint_weight, jitter_range_vec)
        ]
...
if __name__ == "__main__":
    ...
    all_tets = sierpinski_3d(
        generations,
        scale=100,
        midpoint_weight=midpoint_weight,
        jitter_range_vec=jitter_range_vec,
    )
